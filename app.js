...
// View form to HOD
app.get('/hod/driverForm', async (req, res) => {
    try {
        const drivers = await User.find({ role: 'driver' }); // Fetch drivers from the database

        res.render('driverForm', {
            drivers: drivers // Pass drivers to the EJS template
        });
    } catch (error) {
...
app.get('/employee/travel-request-form', async (req, res) => {
    if (req.session.user && req.session.user.role === 'employee') {
        try {
            ...
            // Render the form and pass the user data, including name, role, and department
            res.render('em_TRF', {
                user: {
...
app.get('/hod/TRF_ADMIN', async (req, res) => {
    try {
        if (req.session.user && req.session.user.role === 'hod') {
            const user = await User.findById(req.session.user._id); // Assuming userId is stored in session
            res.render('hod_trf', {
                user, // Send the user data, including department
            });
        } else {
            res.redirect('/');
        }
...
app.get('/api/users/list', async (req, res) => {
    try {
        ...
        const hods = await User.find({ role: 'hod' }); // Fetch HODs
        ...
        res.render('userList', {
            ...
            employeeCount,
            dhodCount,
            hods // Pass the HODs to the template
        });
    } catch (error) {
        console.error(error);
        res.status(500).send('Server Error');
...
app.get('/cab-booking-form', async (req, res) => {
    try {
        const drivers = await User.find({ role: 'driver' });
        res.render('cabBookingForm', { drivers });
    } catch (error) {
        console.error('Error fetching drivers:', error);
        res.status(500).send('Server Error');
    }
